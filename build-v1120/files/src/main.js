...
async function stableOverlaySourcePayload(kind, name, identity) {
  if (kind === 'widget') {
    ...
    return { ...snapshot, theme: themes[name], background: backgrounds[name] };
  }
...
async function ensureStableOverlaySource(kind, name) {
  ...
  try {
    ...
    console.warn('No se pudo sincronizar la fuente HTTPS fija:', error?.message || error);
...
async function stableOverlaySourceStatus(kind, name, force = false) {
  ...
  if (force) return ensureStableOverlaySource(source.kind, source.name);
  const identity = await stableOverlayIdentity();
...
async function mutateEconomy(input = {}) {
  return queueEconomyOperation(async () => {
    ...
    setStreamWidgetState('wallet', {
      type: 'wallet',
      user,
      displayName: account.displayName,
      profilePictureUrl: account.profilePictureUrl || '',
      balance: account.balance,
      currencyName: String(settings.currencyName || 'Lunitas'),
...
async function streamWidgetSnapshot(type, preview = false) {
  ...
  if (normalized === 'wallet') {
    ...
    const recent = [...economy.balances].sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0))[0];
    if (recent) return {
      type: 'wallet', id: `wallet-${recent.user}`, updatedAt: Number(recent.updatedAt || 0),
      user: recent.user, displayName: recent.displayName || recent.user,
...
function streamWidgetHtml(type, token, preview = false, theme = 'lulu', background = 'plain') {
  const normalized = normalizeStreamWidgetType(type);
...
async function streamWidgetInfo(type = 'playlist', forceTunnel = false) {
  ...
  const tunnel = stable.ok ? { status:'ready', message:'HTTPS fijo listo. Esta URL no cambia al reiniciar.', url:stable.url, stable:true } : (forceTunnel ? await ensureOverlayHttpsTunnel(true) : currentOverlayTunnelInfo());
  ...
  overlayServer = http.createServer(async (request, response) => {
    try {
      ...
      if (url.pathname === '/widget') {
        ...
        response.end(streamWidgetHtml(widgetType, token, widgetPreview, widgetTheme, widgetBackground)); return;
      }
    ...
    const installed = await youtubeWindow.webContents.executeJavaScript(`(() => {
      ...
      const playerIsShowingAd = () => {
        ...
        return Boolean(
          ...
          document.querySelector('.ytp-ad-player-overlay:not([style*="display: none"]), .ytp-ad-preview-container:not([style*="display: none"])')
        );
      };
      const reportAdState = () => {
...
function noteMusicPlayerPayload(provider, payload = {}) {
  ...
  } else if (payload.paused === true && win && !win.isDestroyed() && win.isVisible()) {
    // Una pausa realizada dentro del reproductor visible es intencional.
...
ipcMain.on('app:renderer-ready', async (event) => {
  ...
  try {
    const result = await contents.executeJavaScript(`(async()=>{
      ...
      return {ok:results.length>0&&results.every((item)=>item.changed&&item.active&&!item.hidden&&!item.inert&&item.ariaHidden==='false'&&item.display!=='none'&&item.visibleCount===1&&!item.hiddenAncestor)&&previews.fallbackCount===7&&previews.readyCount===7&&previews.frameSources>=1&&previews.cspAllowsLocalFrames,results,previews};
    })()`, true);
