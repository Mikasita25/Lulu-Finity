...
assert lock["packages"][""]["version"] == "1.1.2"
assert 'id="versionLabel">v1.1.2' in html
assert 'id="updateVersionBadge">v1.1.2' in html
assert "## 1.1.2" in read("CHANGELOG.md")
...
for token in (
    ...
    ".overlay-preview-fallback", ".preview-ready iframe", "sample-overlay-in",
    ".update3-title-pill", ".primary::after", "update3-panel-in",
):
...
for token in (
    ...
    "markPermanentPreviewLoading(frame)",
):
...
for token in (
    ...
    "settingsExistedBeforeInitialization", "widget-enter", "disc-spin",
...
for token in ("overlay-enter", "progress-flow", "gift-float", "prefers-reduced-motion"):
...
print(
    f"Lulu Finity 1.1.2 validada: {len(audit.ids)} IDs únicos, HTTPS fijo, "
    "recuperación musical y avisos separados de instalación/actualización"
)
