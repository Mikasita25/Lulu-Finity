...
const navigationMarkerPath = path.join(temporaryRoot, 'navigation-ready.json');
process.env.CI = 'true';
...
class FakeWebContents extends EventEmitter {
  ...
  async executeJavaScript(source) {
    this.lastExecutedSource = source;
    return { ok:true, results:[{ name:'dashboard', visibleCount:1 }] };
  }
}
...
(async () => {
  try {
    ...
    assert.equal(FakeBrowserWindow.instances[0].options.webPreferences.backgroundThrottling, false, 'El bot de voz quedó limitado al minimizar Lulu');
    ...
    assert.equal(initial.version, '1.0.5');
    ...
    assert.equal(typeof setActivePage, 'function');
    ...
    for (const [page, moduleName] of pageModules) {
      const activated = await setActivePage(null, page);
      const runtime = await runtimeStatus();
      assert.equal(activated.module, moduleName, `${page} activó un módulo incorrecto`);
      assert.equal(runtime.modules.visible, moduleName, `${page} no quedó como zona visible`);
      assert.deepEqual([...runtime.modules.active].sort(), ['core', moduleName].sort(), `${page} dejó otra zona activa`);
    }
    for (const page of ['about', 'settings', 'dashboard']) {
      const activated = await setActivePage(null, page);
      const runtime = await runtimeStatus();
      assert.equal(activated.module, null, `${page} no debería activar un módulo adicional`);
      assert.equal(runtime.modules.visible, null);
      assert.deepEqual(runtime.modules.active, ['core']);
    }
    ...
    delete process.env.LULU_STARTUP_SMOKE_MARKER;
    process.env.LULU_NAVIGATION_SMOKE_MARKER = navigationMarkerPath;
    await rendererReady({ sender:FakeBrowserWindow.instances[0].webContents });
    const navigation = JSON.parse(fs.readFileSync(navigationMarkerPath, 'utf8'));
    assert.equal(navigation.ok, true);
    assert.match(FakeBrowserWindow.instances[0].webContents.lastExecutedSource, /goToPage\(name/);
    assert.equal(FakeBrowserWindow.instances.length, 1, 'El arranque abrió reproductores auxiliares');
    console.log(`Arranque 1.0.5 validado: ${ipcHandlers.size} handlers, ${pageModules.size + 3} páginas con ciclo exclusivo y navegación lista`);
  } finally {
