...
function renderTikTokChatStatus(status = state.tiktokChatStatus || {}) {
  ...
  if (badge) {
    badge.textContent = status.ready ? 'LISTO' : status.requiresLogin ? 'INICIA SESIÓN' : status.open ? 'ABIERTO' : 'SIN SESIÓN';
    badge.classList.toggle('ready', Boolean(status.ready));
    badge.classList.toggle('warning', Boolean(!status.ready && status.open));
  }
  if (text) text.textContent = status.message || (status.ready ? 'TikTok está listo para enviar mensajes.' : 'Abre TikTok e inicia sesión con la cuenta creadora del LIVE.');
}
...
function normalizedEventMediaRules() {
  return (Array.isArray(state.settings?.eventMediaRules) ? state.settings.eventMediaRules : [])
    .map((rule) => ({
      ...
      mediaPath: String(rule?.mediaPath || ''),
      mediaName: String(rule?.mediaName || 'Sonido'),
...
async function refreshRankingInfo(slot = null, refreshFrame = false) {
  ...
  try {
    ...
    if($('rankingHttpsStatus')){const ready=Boolean(info.url);$('rankingHttpsStatus').textContent=ready?'HTTPS listo. Mantén Lulu abierta; el enlace puede cambiar al reiniciarla.':(info.tunnelMessage||'No se pudo crear HTTPS. Pulsa Copiar HTTPS para reintentar.');$('rankingHttpsStatus').classList.toggle('ready',ready);$('rankingHttpsStatus').classList.toggle('error',!ready);}
    ...
    if(frame&&(refreshFrame||!frame.src||!frame.src.includes(`/ranking?slot=${selected}`))) frame.src=`${info.previewUrl}&refresh=${Date.now()}`;
...
async function refreshStreamWidgetInfo(type, refreshFrame=false) {
  ...
  try {
    ...
    if(frame&&(refreshFrame||!frame.src||!frame.src.includes(`/widget?type=${normalized}`)))frame.src=`${info.previewUrl}&refresh=${Date.now()}`;
    return info;
  } catch(error) {
...
function parseVoiceValue(value) {
  ...
  if (raw.startsWith('local:')) return { mode:'local', localVoiceId:raw.slice(6), voiceURI:'', onlineVoice:'' };
...
function renderVoiceOptions() {
  ...
  if (systemMatches.length) {
    ...
    group.label = `Voces de Windows (${systemMatches.length})`;
  ...
  const onlineMatches = state.onlineVoices.filter((voice) => (languageMatches(voice.locale, filter) || `online:${voice.shortName}` === selected) && (!search || normalizeText(`${voice.localName} ${voice.name} ${voice.shortName} ${voice.locale} ${voice.gender}`).includes(search) || `online:${voice.shortName}` === selected));
  ...
  if ($('voiceVisibleCount')) $('voiceVisibleCount').textContent = `${localMatches.length+systemMatches.length+onlineMatches.length} visibles de ${state.localVoices.length+state.voices.length+state.onlineVoices.length} voces`;
  ...
  if ([...select.options].some((option) => option.value === selected)) {
    ...
    if(preferredLocal){select.value=`local:${preferredLocal.id}`;state.settings.voiceMode='local';state.settings.localVoiceId=preferredLocal.id;}
      ...
      select.value = `system:${preferredSystem.voiceURI}`;
      state.settings.voiceMode = 'system';
      state.settings.voiceURI = preferredSystem.voiceURI;
    }
  }
}
...
async function loadOnlineVoices(showToast = false) {
  ...
  try {
    ...
    state.onlineVoices = Array.isArray(result?.voices) ? result.voices : [];
    state.onlineVoicesFallback = Boolean(result?.fallback);
    status.className = `voice-provider-status ${state.onlineVoices.length ? 'ready' : 'error'}`;
    status.innerHTML = state.onlineVoices.length
      ...
      : '<span class="status-light error"></span><span>No se pudieron cargar las voces online. Las voces de Windows siguen disponibles.</span>';
    renderVoiceOptions();
    ...
    status.innerHTML = '<span class="status-light error"></span><span>No se pudieron cargar voces online. Usa una voz instalada en Windows.</span>';
    renderVoiceOptions();
...
function setStatus(payload) {
  ...
  if (!wasConnected && state.connected) void sendTikTokAutoChatEvent('liveConnected', { usuario:payload?.username || state.settings?.username || '' });
...
function normalizedCommands() {
  return (Array.isArray(state.settings.customCommands) ? state.settings.customCommands : [])
    ...
    .map((command) => ({
      ...
      mediaUrl: String(command.mediaUrl || ''),
      mediaPath: String(command.mediaPath || ''),
      mediaName: String(command.mediaName || ''),
...
function enqueueSpeech(message, options = {}) {
  ...
  if (!hasAudienceAccess(message, state.settings.ttsPermissionMode || 'all', 'tts')) {
    return fail(permissionDeniedLabel(state.settings.ttsPermissionMode, 'tts'));
  }
  if (lockKey && options.lockReserved !== true && !reserveAudioLock(lockKey)) {
    return { added: false, reason: 'comando en cooldown hasta que termine el audio' };
  }
  const queueLimit = Math.max(1, Number(state.settings.queueLimit) || 30);
  if (state.speechQueue.length >= queueLimit) {
    releaseAudioLock(lockKey);
    toast('Cola de voz llena', 'Se omitió un comentario para evitar retraso.', 'error');
    return { added: false, reason: 'cola de voz llena' };
  }
  state.speechQueue.push({
    id: message.id,
    text: makeSpeechText(message),
    voice: voiceForMessage(message),
    audioLockKey: lockKey
  });
  renderStats();
  speakNext();
  return { added: true };
...
function startSystemSpeechNow(text, voiceConfig, token, finish) {
  ...
  utterance.onstart = () => { if (token === state.speechToken) renderStats(); };
...
function runSpeechNow(text, isQueue = false, queueId = null, voiceConfig = null) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (success) => {
      ...
      if (queueId) updateCommentResult(queueId, success ? 'read' : 'skipped', success ? '' : 'audio detenido');
      renderStats();
      if (isQueue) setTimeout(speakNext, 70);
    ...
    if (voiceMode === 'system') {
      startSystemSpeechNow(text, voiceConfig, token, finish);
      return;
    }
    if(voiceMode==='local'){
      (async()=>{try{const tuning=speechTuning(voiceConfig);const result=await api.synthesizeLocalVoice({text,voiceId:voiceConfig?.localVoiceId||state.settings.localVoiceId||'lulu-es-mx',speed:tuning.rate,idleMinutes:state.settings.localVoiceIdleMinutes||2});if(finished||token!==state.speechToken){finish(false);return;}const audio=new Audio(`data:${result.mimeType||'audio/wav'};base64,${result.data}`);audio.volume=tuning.volume;state.onlineAudio=audio;audio.onended=()=>finish(true);audio.onerror=()=>finish(false);audio.onabort=()=>finish(false);await audio.play();}catch(error){if(finished||token!==state.speechToken){finish(false);return;}toast('Lulu Local no disponible','Se usó una voz del sistema para este audio.','error');startSystemSpeechNow(text,{...voiceConfig,mode:'system'},token,finish);}})();return;
    }
    ...
    (async () => {
      try {
        ...
        startSystemSpeechNow(text, { ...voiceConfig, mode: 'system' }, token, finish);
      }
    })();
  });
}
...
function speakText(text, isQueue = false, queueId = null, voiceConfig = null, options = {}) {
  ...
  return enqueueExclusiveAudio(
    () => runSpeechNow(text, isQueue, queueId, voiceConfig),
    { ...options, kind: 'speech', label: options.label || 'Voz TTS' }
  );
}
...
function speakNext() {
  ...
  state.speaking = true;
  renderStats();
  const queued = speakText(item.text, true, item.id, item.voice, {
    lockKey: item.audioLockKey || '',
    lockReserved: Boolean(item.audioLockKey),
    label: item.audioLockKey ? 'Comando TTS' : 'Comentario TTS'
  });
  if (!queued.accepted) {
...
function normalizedAutomationRules(){return Array.isArray(state.settings?.automationRules)?state.settings.automationRules:[]}
...
async function refreshOverlayInfo(screen = null) {
  ...
  try {
    ...
    if ($('overlayHttpsStatus')) { const ready=Boolean(info.url); $('overlayHttpsStatus').textContent=ready?'HTTPS listo. Mantén Lulu abierta; el enlace puede cambiar al reiniciarla.':(info.tunnelMessage||'No se pudo crear HTTPS. Pulsa Copiar HTTPS para reintentar.'); $('overlayHttpsStatus').classList.toggle('ready',ready); $('overlayHttpsStatus').classList.toggle('error',!ready); }
...
function updateCommandMediaFields() {
  ...
  $('pickCommandMediaBtn').textContent = action === 'image' ? 'Elegir imagen' : 'Elegir sonido';
...
function openCommandModal(id = '') {
  const command = normalizedCommands().find((item) => item.id === id) || {
    ...
    mediaUrl: '', mediaPath: '', mediaName: '', mediaVolume: 0.9, mediaDuration: 6, overlayScreen: 1, cost: 0
  };
  state.commandMediaDraft = command.mediaUrl ? { type: command.action === 'image' ? 'image' : 'audio', url: command.mediaUrl, path: command.mediaPath, name: command.mediaName } : null;
...
async function refreshRuntimeStatus(){if(!$('runtimeStats'))return;try{const runtime=await api.getRuntimeStatus();$('runtimeStats').innerHTML=`<div class="runtime-stat"><strong>${Number(runtime.memoryMb||0)} MB</strong><span>Memoria principal</span></div><div class="runtime-stat"><strong>${Number(runtime.processes||0)}</strong><span>Procesos</span></div><div class="runtime-stat"><strong>${Number(runtime.modules?.overlayClients||0)}</strong><span>Fuentes en pantalla</span></div>`;const modules=[['LIVE',runtime.modules?.live],['Lulu Local',runtime.modules?.localTts?.running],['YouTube',runtime.modules?.youtube],['Spotify',runtime.modules?.spotify],['Overlays',runtime.modules?.overlayServer]];$('runtimeModules').innerHTML=modules.map(([label,on])=>`<span class="runtime-module ${on?'live':''}">${label} · ${on?'activo':'en espera'}</span>`).join('');}catch(error){$('runtimeStats').textContent=error.message||String(error);}}
...
function activatePageModules(page){if(page==='voice'){void loadLocalVoices();if(state.settings?.voiceMode==='online')void loadOnlineVoices(false);}if(page==='rankings'&&!state.loadedPages.has(page)){void refreshOverlayInfo(state.overlay?.screen||1);state.ranking.slot=clamp(state.ranking?.slot||1,1,4);setRankingControlValues();void refreshRankingInfo(state.ranking.slot,true);}if(page==='automations'&&!state.loadedPages.has(page)){for(const type of ['alert','goal','gift'])void refreshStreamWidgetInfo(type,true);}if(page==='settings'&&!state.loadedPages.has(page))void refreshRelayUsage();if(page==='account'&&!state.loadedPages.has(page))void api.getTikTokChatStatus().then(renderTikTokChatStatus).catch(()=>{});state.loadedPages.add(page);}
...
function setupEvents() {
  ...
  $('pickCommandMediaBtn').addEventListener('click', async () => {
    const kind = $('commandActionInput').value === 'image' ? 'image' : 'audio';
    const selected = await api.pickMedia(kind);
    if (!selected) return;
    state.commandMediaDraft = selected;
  ...
  $('pickEventSoundBtn')?.addEventListener('click', async () => {
    ...
    if (!selected) return;
    state.eventMediaDraft = selected;
    $('eventSoundName').textContent = selected.name;
  });
...
async function init() {
  ...
  state.settings.eventMediaRules = Array.isArray(state.settings.eventMediaRules) ? state.settings.eventMediaRules : [];
  ...
  renderDashboardMusic();
  if (migratedV100||migratedDefaultCommands || migratedVoiceCatalog || !state.settings.rankingsMigratedV016 || !state.settings.streamWidgetsMigratedV019) { state.settings.rankingsMigratedV016=true; state.settings.streamWidgetsMigratedV019=true; state.settings = await api.saveSettings(state.settings); }
  setStatus({ status: 'disconnected' });
  setupEvents();
