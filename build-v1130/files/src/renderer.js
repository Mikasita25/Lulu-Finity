...
const RELEASE_NOTES = Object.freeze({
  '1.1.3': Object.freeze([
    Object.freeze({ icon:'◈', title:'Editor de widgets', text:'Personaliza colores, fondo, transparencia, bordes y la barra de metas.' }),
    Object.freeze({ icon:'🔒', title:'HTTPS igual a la vista previa', text:'Los estilos propios ahora se guardan y se reflejan en OBS y TikTok LIVE Studio.' }),
    Object.freeze({ icon:'◻', title:'Overlays sin animaciones', text:'Se eliminaron movimientos continuos para reducir el uso de recursos.' })
  ]),
  '1.1.2': Object.freeze([
...
function scheduleStreamWidgetStyleRefresh(type) {
  ...
  state.streamWidgetStyleRefreshTimers[type] = setTimeout(async () => {
    const info = await refreshStreamWidgetInfo(type, true);
    if (status) status.textContent = info?.url ? 'Guardado · HTTPS actualizado.' : 'Guardado · HTTPS se reintentará automáticamente.';
  }, 700);
}
