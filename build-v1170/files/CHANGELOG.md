...
- Reconoce comandos aunque TikTok entregue espacios no separables, caracteres invisibles, signo `！` o `¡`, mayúsculas, acentos o separadores como `:` y `;`.
- Mantiene la coincidencia segura: el comando debe iniciar el comentario y no acepta nombres parciales.
- Añade `!true` activado por defecto para continuar la música y `!stop` desactivado por defecto para pausarla; ambos pueden editarse como los demás comandos.
- Corrige los comandos con costo: activa Economía al guardar o migrar un comando con precio y bloquea la ejecución si el cobro no puede realizarse.
- Muestra Monedas únicamente durante ocho segundos al consultar `!saldo` y Juegos sólo mientras existe una partida o su resultado reciente.
- Oculta Alertas y Regalos al vencer su acción; las Metas permanecen visibles solamente cuando están habilitadas.
- Aplica el mismo comportamiento condicional en las fuentes locales y HTTPS de OBS/TikTok LIVE Studio.
- Evita que YouTube se limite en segundo plano, conecta la recuperación a la ventana musical correcta y reanuda pausas inesperadas.
- Distingue una pausa manual real en YouTube o Spotify para no deshacerla.

## 1.1.6

- Activa automáticamente la fuente HTTPS al abrir cada apartado de widgets, rankings o superposiciones.
- Evita que una fuente nueva se quede indefinidamente en “HTTPS no disponible” hasta pulsar el botón de copiar.
- Conserva la misma URL pública después de reiniciar y vuelve a sincronizar el contenido en segundo plano.
- Muestra el fallo real del enlace seguro y mantiene el reintento periódico si Railway, Internet o el firewall no responden.
- Conserva las correcciones de controles de ventana, personalización y música bajo demanda de 1.1.5.

## 1.1.5

- Separa los controles de ventana del arranque del renderer para que cerrar, minimizar y maximizar sigan respondiendo ante un fallo de interfaz.
- Evita que el aviso de actualización cubra la barra superior y que una biblioteca de sonidos dañada detenga toda la aplicación.
- Corrige la selección de temas, fondos y estilos para los seis widgets: primero guarda, después actualiza la muestra local y finalmente confirma el HTTPS correspondiente.
- Activa o repara automáticamente la fuente HTTPS al personalizar un widget y conserva su misma dirección pública.
- Evita recargas adelantadas del editor de colores que podían mostrar el diseño anterior.
