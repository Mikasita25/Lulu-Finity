...
test('el widget musical queda transparente hasta recibir una solicitud', () => {
  const html = renderOverlayPage('c'.repeat(32), 'widget', 'playlist');
  assert.match(html, /data\.visible===false/);
  assert.match(html, /!data\.current&&!items\.length/);
  assert.match(html, /hideRoot\(\);return/);
});

test('los widgets de actividad esperan un evento vigente', () => {
  for (const widget of ['wallet', 'game', 'alert', 'gift']) {
    const html = renderOverlayPage('d'.repeat(32), 'widget', widget);
    assert.match(html, /timedActivityVisible/);
    assert.match(html, /expiresAt>Date\.now\(\)/);
    assert.match(html, /armActivityExpiry/);
  }
  const goal = renderOverlayPage('e'.repeat(32), 'widget', 'goal');
  assert.match(goal, /data\.visible===false\|\|!data\.title/);
});
