...
function replaceRegexOnce(source, regex, replacement, label) {
  ...
  if (matches.length !== 1) throw new Error(`No se pudo aplicar ${label}: se esperaba 1 coincidencia y hubo ${matches.length}.`);
...
`;
  ...
  source = replaceOnce(source, "function advanceQueue({ natural = false } = {}) {\n  const previous = playback.current;", "function advanceQueue({ natural = false } = {}) {\n  const previous = playback.current;\n  resetYoutubeRecovery();\n  resetAudiusResolveRetry();", 'reinicio al avanzar');
  source = replaceOnce(source, "async function startCurrentSong() {\n  const item = playback.current;\n  if (!item) return;\n  const nonce = ++playerNonce;", "async function startCurrentSong() {\n  const item = playback.current;\n  if (!item) return;\n  const nonce = ++playerNonce;\n  resetYoutubeRecovery(item);\n  resetAudiusResolveRetry(item);", 'reinicio por canción');
  source = replaceOnce(source, "async function fallbackAudiusToYoutube(reason, nonce) {\n  const item = playback.current;\n  if (!item || item.provider !== 'audius' || nonce !== playerNonce) return;", "async function fallbackAudiusToYoutube(reason, nonce) {\n  const item = playback.current;\n  if (!item || item.provider !== 'audius' || nonce !== playerNonce) return;\n  if (await retryAudiusSameTrack(reason, nonce)) return;", 'reintento Audius');
  ...
  source = replaceOnce(source, "        playback.currentTime = Number(data.currentTime) || 0; playback.duration = Number(data.duration) || 0;\n        broadcastState();", "        playback.currentTime = Number(data.currentTime) || 0; playback.duration = Number(data.duration) || 0;\n        if (!playback.paused && playback.currentTime > 1 && youtubeRecovery.itemId === playback.current.id) youtubeRecovery.attempts = 0;\n        broadcastState();", 'progreso YouTube');
  source = replaceOnce(source, "    const ended = message.match(/^__LULU_MUSIC_ENDED__:(\\d+)$/);\n    if (ended && Number(ended[1]) === playerNonce) advanceQueue({ natural:true });", "    const stalled = message.match(/^__LULU_MUSIC_STALLED__:(\\d+):([0-9.]+)$/);\n    if (stalled && Number(stalled[1]) === playerNonce) { void recoverYoutubePlayback('buffering prolongado', playerNonce, Number(stalled[2]) || playback.currentTime); return; }\n    const ended = message.match(/^__LULU_MUSIC_ENDED__:(\\d+)$/);\n    if (ended && Number(ended[1]) === playerNonce) advanceQueue({ natural:true });", 'evento stalled YouTube');
  ...
  source = replaceRegexOnce(
    source,
    /async function installYoutubeWatcher\(nonce, attempt\) \{[\s\S]*?\n\}\n\nfunction activePlayerWindow/,
    `${newWatcher}\n\nfunction activePlayerWindow`,
    'watchdog YouTube'
  );
