...
export function MusicPlaybackHost() {
  ...
  const setPlaybackStatus = useMobileControlStore(
  ...
  const musicActive =
    music.enabled && (Boolean(currentSong) || browser.playing);
  ...
  const navigate = () => {
    ...
    setAutoSelect(false);
    useMobileControlStore.setState({
  ...
  useEffect(() => {
    void setAudioModeAsync({
      ...
      console.warn('[LuluFinity] background audio mode failed', error),
    );
  }, []);
  ...
  return (
    ...
    >
      <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
        <View
          style={{
            display: browser.open ? 'flex' : 'none',
            padding: 12,
            gap: 10,
          }}
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <Pressable
              accessibilityLabel="Minimizar navegador"
              onPress={browser.hide}
              style={{ padding: 10 }}
            >
              <ChevronDown color="#C5B5FF" size={24} />
            </Pressable>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <View style={{ flex: 1 }}>
              <LuluInput
                ...
                placeholder="Buscar o pegar enlace de YouTube"
                autoCapitalize="none"
                autoCorrect={false}
                returnKeyType="search"
                accessibilityLabel="Buscar en YouTube"
                icon={<Search color={palette.pinkSoft} size={18} />}
              />
            </View>
            <Pressable
              onPress={navigate}
            ...
            >
              <Search color={palette.pinkSoft} size={20} />
            </Pressable>
          </View>
          <View
            style={{
          ...
          >
            <Pressable
              ...
              style={{ padding: 12, opacity: canBack ? 1 : 0.3 }}
            >
            ...
            <Pressable
              ...
              style={{ padding: 12, opacity: canForward ? 1 : 0.3 }}
            >
            ...
            <Pressable
              ...
              accessibilityLabel={playbackPaused ? 'Continuar' : 'Pausar'}
              style={{ padding: 12 }}
            >
              {playbackPaused ? (
                <Play color="#C5B5FF" size={20} />
              ) : (
                <Pause color="#C5B5FF" size={20} />
              )}
            </Pressable>
            <Pressable
              accessibilityLabel="Cambiar modo de escritorio"
              onPress={() => {
                setDesktop(!desktop);
              }}
              style={{ padding: 10 }}
            >
              <Text style={{ color: '#C5B5FF', fontSize: 11 }}>
                {desktop ? 'Escritorio' : 'Móvil'}
              </Text>
            </Pressable>
            <Text style={{ color: '#ADB4CE', fontSize: 11 }}>
              {pageLoading
                ? 'Cargando…'
                : music.backgroundPlayback
                  ? 'Segundo plano habilitado'
                  : 'Solo en pantalla'}
            </Text>
          </View>
          {browserError ? (
            <Text accessibilityRole="alert" style={{ color: '#FFABBA' }}>
              {browserError}
            </Text>
          ) : null}
        </View>
        <WebView
          ...
          onShouldStartLoadWithRequest={(request) =>
            request.url === 'about:blank' || isYouTubeUrl(request.url)
          }
          ...
          onLoadStart={() => {
            ...
            loadTimer.current = setTimeout(() => {
              ...
              setBrowserError(
                'YouTube tarda en responder. Puedes recargar la página.',
              );
              setPlaybackStatus(
                'error',
          ...
          onLoadEnd={() => {
            ...
            loadTimer.current = setTimeout(() => {
              ...
              if (
                ...
                setPlaybackStatus(
                  'error',
                  'YouTube no inició la canción. Toca Reintentar.',
                );
              }
            }, 15_000);
          }}
          onMessage={(event) => {
            try {
              ...
              if (
                message?.type === 'user-pause' ||
                message?.type === 'user-play'
              ) {
                useMobileControlStore
                  .getState()
                  .setPlaybackPaused(message.type === 'user-pause');
              ...
              if (message?.type === 'ended') {
                ...
                if (autoSelect && !playNextSong())
                  useMobileControlStore.getState().setPlaybackPaused(true);
              }
              if (message?.type === 'video-error') {
                setPlaybackStatus(
                  'error',
                  'YouTube no pudo reproducir este video.',
                );
              }
            } catch {}
          }}
          onError={() => {
            ...
            setPageLoading(false);
            setBrowserError(
              'No se pudo cargar YouTube. Revisa la conexión y toca recargar.',
            );
            browser.setPlaying(false);
            setPlaybackStatus(
              'error',
              'No se pudo conectar con YouTube. Toca Reintentar.',
            );
          }}
          onRenderProcessGone={() => {
            browser.setPlaying(false);
            setBrowserError(
              'El navegador se cerró. Toca recargar para recuperarlo.',
            );
            setPlaybackStatus('error', 'El navegador necesita recargarse.');
          }}
