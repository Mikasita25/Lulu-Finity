...
import { Gift, Heart, MessageCircle, Share2, Sparkles, Star, UserPlus, Users } from 'lucide-react-native';
...
export function EventRow({ event }: { event: LiveEvent }) {
  ...
  return (
    <View className="flex-row items-center gap-3 border-b border-white/[0.055] py-3.5">
      {event.type === 'fanSticker' && event.fanStickerImageUrl ? (
        <Image source={{ uri: event.fanStickerImageUrl }} className="h-11 w-11 rounded-2xl bg-white/10" />
      ...
      <View className="flex-1">
        ...
        <Text
          ...
          className={`mt-1 text-xs leading-5 ${
            event.type === 'comment' || event.type === 'fanSticker' ? 'text-white/75' : 'text-white/50'
          }`}
          numberOfLines={event.type === 'comment' || event.type === 'fanSticker' ? 2 : 1}
        >
          {eventText(event)}
        </Text>
      </View>
