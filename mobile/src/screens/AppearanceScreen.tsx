...
const themes: { id: AccentTheme; label: string; color: string }[] = [
  { id: 'lulu', label: 'Lulú Dream', color: '#D685FF' },
  { id: 'violet', label: 'Lavanda', color: '#A762FF' },
  { id: 'rose', label: 'Rosa lunar', color: '#F08CB6' },
  { id: 'cyan', label: 'Menta noche', color: '#74DDE4' },
];
...
export function AppearanceScreen() {
  ...
  return (
    <Screen>
      <AppHeader
        title="Apariencia"
        subtitle="Elige un color sencillo para toda la aplicación."
      />

      <SectionTitle
        title="Color principal"
        subtitle="La interfaz oscura se conserva para que sea cómoda mientras juegas."
      />
      <GlassCard variant="hero">
        <View className="p-5">
          <View className="flex-row items-center gap-3">
            <Palette size={20} color="#F2B7FF" />
            <View className="flex-1">
              <Text className="text-sm font-black text-white">Tema oscuro</Text>
              <Text className="mt-1 text-xs leading-5 text-white/60">
                Cielo nocturno y capas de vidrio; solo cambia el acento.
              </Text>
            </View>
