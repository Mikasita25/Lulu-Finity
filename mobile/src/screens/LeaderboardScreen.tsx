...
const metrics: { id: Metric; label: string }[] = [
  ...
  { id: 'fanStickers', label: 'Fan Stickers' },
...
function Avatar({ entry, size = 52 }: { entry: LeaderboardEntry; size?: number }) {
  if (entry.profilePictureUrl) {
    return <Image source={{ uri: entry.profilePictureUrl }} style={{ width: size, height: size, borderRadius: size / 2 }} className="bg-white/10" />;
  }
  return (
    <View style={{ width: size, height: size, borderRadius: size / 2 }} className="items-center justify-center bg-lulu-500/20">
...
export function LeaderboardScreen() {
  ...
  const entries = useMemo(
    () => Object.values(leaderboard).filter((entry) => valueFor(entry, metric) > 0).sort((a, b) => valueFor(b, metric) - valueFor(a, metric)),
    [leaderboard, metric],
  );
  const top = entries.slice(0, 3);
  const rest = entries.slice(3);
  const fontFamily = rankingFont === 'mono' ? 'monospace' : rankingFont === 'rounded' ? 'sans-serif-medium' : undefined;
        ...
        <View className="p-5">
          ...
          <View className="mt-7 flex-row items-end justify-center gap-2">
            {[top[1], top[0], top[2]].map((entry, slot) => {
              if (!entry) return <View key={`empty-${slot}`} className={slot === 1 ? 'h-44 flex-1' : 'h-36 flex-1'} />;
              const rank = slot === 1 ? 1 : slot === 0 ? 2 : 3;
              return (
                <Animated.View key={entry.uniqueId} entering={FadeInUp.delay(rank * 80).springify().damping(16)} className={`flex-1 items-center rounded-3xl border border-white/10 bg-white/[0.045] px-2 pb-4 ${rank === 1 ? 'pt-5' : 'pt-4'}`}>
                  {rank === 1 ? <Crown size={21} color="#FFE07D" /> : <Medal size={18} color="#FFB8E5" />}
                  <View className="mt-2"><Avatar entry={entry} size={rank === 1 ? 58 : 48} /></View>
                  {rankingRgb ? (
                    <RgbName fontFamily={fontFamily} className="mt-2 max-w-full text-center text-xs font-black">{entry.nickname}</RgbName>
                  ) : (
                    <Text style={{ color: rankingTextColor, fontFamily }} className="mt-2 max-w-full text-center text-xs font-black" numberOfLines={1}>{entry.nickname}</Text>
                  )}
                  <Text className="mt-1 text-[11px] font-extrabold text-lulu-200">{compactNumber(valueFor(entry, metric))}</Text>
                </Animated.View>
              );
            })}
          </View>
