  ...
  [
    {
      type: 'comment',
      label: 'Comentarios',
      subtitle: 'Mensajes normales del chat',
    },
    { type: 'gift', label: 'Regalos', subtitle: 'Regalos y repeticiones' },
    {
      type: 'follow',
      label: 'Nuevos seguidores',
      subtitle: 'Usuarios que empezaron a seguir',
    },
    {
      type: 'share',
      label: 'Compartidos',
      subtitle: 'Usuarios que compartieron el LIVE',
    },
    {
      type: 'subscribe',
      label: 'Suscripciones',
      subtitle: 'Nuevas suscripciones del LIVE',
    },
    {
      type: 'fanSticker',
      label: 'Fan Stickers',
      subtitle: 'Stickers de Fan y Super Fan',
    },
    {
      type: 'like',
      label: 'Likes',
      subtitle: 'Eventos de likes; pueden ser muy frecuentes',
    },
    {
      type: 'member',
      label: 'Entradas al LIVE',
      subtitle: 'Entradas y cambios de audiencia',
    },
  ];
    ...
    <Screen>
        ...
        <View className="p-5">
          <View className="flex-row items-center gap-3">
            <View className="h-11 w-11 items-center justify-center rounded-2xl bg-lulu-500/10">
              <SlidersHorizontal size={19} color="#F2B7FF" />
            </View>
            <View className="flex-1">
              <Text className="text-sm font-black text-white">
                {enabledCount} tipos activos
              </Text>
          ...
          <View className="mt-4 flex-row gap-2">
            ...
            <Pressable
              ...
              className="flex-1 flex-row items-center justify-center gap-2 rounded-2xl bg-white/[0.06] px-3 py-3"
            >
              <EyeOff size={16} color="#B7A9B4" />
              <Text className="text-xs font-black text-white/55">
                Ocultar todo
              </Text>
            </Pressable>
          </View>
      ...
      <GlassCard>
        <View className="px-5">
          {FILTERS.map((item) => (
            <View
              key={item.type}
              className="flex-row items-center gap-3 border-b border-white/[0.055] py-4"
            >
              <View className="flex-1">
                <Text className="text-sm font-black text-white">
                  {item.label}
      ...
      <View className="mb-2 flex-row gap-2">
        {[10, 25, 50].map((value) => (
          ...
          >
            <Text
              className={`text-center text-xs font-black ${maxItems === value ? 'text-white' : 'text-white/65'}`}
            >
              {value}
            </Text>
          </Pressable>
