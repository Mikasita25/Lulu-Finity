...
const slots: { id: SoundSlot; label: string; hint: string }[] = [
  ...
  { id: 'follow', label: 'Nuevo follower', hint: 'Para nuevos seguidores.' },
  {
    id: 'like',
    label: 'Likes',
    hint: 'Opcional; desactivado por defecto para evitar spam.',
  },
  {
    id: 'share',
    label: 'Compartido',
    hint: 'Cuando alguien comparte el LIVE.',
  },
  {
    id: 'comment',
    label: 'Comentario',
    hint: 'Opcional para mensajes del chat.',
  },
  {
    id: 'fanSticker',
    label: 'Fan Sticker',
    hint: 'Para uno distinto por sticker usa Automatizaciones.',
  },
  {
    id: 'member',
    label: 'Entrada al LIVE',
    hint: 'Cuando entra un espectador.',
  },
  {
    id: 'subscribe',
    label: 'Suscripción',
    hint: 'Para nuevas suscripciones del LIVE.',
  },
  { id: 'goal', label: 'Meta completada', hint: 'Sonido de celebración.' },
  {
    id: 'rank',
    label: 'Cambio de ranking',
    hint: 'Suena al cambiar el Top 3.',
  },
];
...
export function SoundsScreen() {
  ...
  return (
    <Screen>
      <AppHeader
        title="Sonidos"
        subtitle="Biblioteca, mezcla y alertas personalizadas."
      />
        ...
        <View className="p-5">
          <View className="flex-row items-center gap-3">
            ...
            <View className="flex-1">
              ...
              <Text className="mt-1 text-xs leading-5 text-white/60">
                Ajusta todas las alertas sin editar una por una.
              </Text>
            </View>
          </View>
          ...
          <View className="flex-row gap-2">
            {profiles.map((profile) => {
              ...
              return (
                ...
                >
                  <Text
                    className={`text-center text-xs font-black ${active ? 'text-lulu-100' : 'text-white/55'}`}
                  >
                    {profile.label}
                  </Text>
                  <Text className="mt-1 text-center text-[9px] text-white/25">
                    {profile.hint}
                  </Text>
                </Pressable>
              );
          ...
          <View className="mt-5">
            <LuluSlider
              ...
              value={soundMix.masterVolume}
              onValueChange={(masterVolume) => setSoundMix({ masterVolume })}
              step={0.05}
              disabled={readonly}
            />
          </View>
          ...
          <View className="mt-5 flex-row items-center gap-3 border-t border-white/[0.06] pt-4">
            <Music2 size={18} color="#F2B7FF" />
            <View className="flex-1">
              <Text className="text-xs font-black text-white">
                Bajar música durante alertas
              </Text>
              <Text className="mt-1 text-[10px] leading-4 text-white/60">
                La canción continúa; solo reduce su volumen temporalmente.
              </Text>
            </View>
            <LuluSwitch
              disabled={readonly}
              ...
              accessibilityLabel="Bajar música durante alertas"
            />
          </View>
          ...
          <View className="mt-4 flex-row items-center gap-3">
            ...
            <View className="flex-1">
              <Text className="text-xs font-black text-white">
                Permitir sonidos simultáneos
              </Text>
              <Text className="mt-1 text-[10px] leading-4 text-white/60">
                Útil para LIVE rápidos; el perfil Impacto lo activa.
              </Text>
            </View>
            <LuluSwitch
              disabled={readonly}
              ...
              accessibilityLabel="Permitir sonidos simultáneos"
            />
          </View>
        </View>
      </GlassCard>
      ...
      {readonly ? (
        <Text className="mb-4 mt-4 rounded-2xl bg-white/[0.05] p-4 text-xs leading-5 text-white/60">
          Modo Espectador: puedes probar los sonidos, pero la configuración solo
          se edita en modo Streamer.
        </Text>
      ) : null}

      <Text className="mb-3 mt-6 text-xs font-black uppercase tracking-[1.5px] text-white/60">
        Sonido por evento
      </Text>
      {slots.map((slot) => {
        ...
        return (
          <GlassCard key={slot.id} className="mb-3">
            <View className="p-5">
              ...
              {!readonly ? (
                <>
                  <Text className="mb-2 mt-4 text-[10px] font-black uppercase tracking-[1.4px] text-white/55">
                    Biblioteca incluida
                  </Text>
                  <View className="flex-row flex-wrap gap-2">
                    {BUILTIN_SOUNDS.map((preset) => {
                      ...
                      return (
                        ...
                        >
                          <Text
                            className={`text-[10px] font-black ${active ? 'text-lulu-100' : 'text-white/50'}`}
                          >
                            {preset.name}
                          </Text>
                        </Pressable>
              ...
              <View className="mt-4">
                <LuluSlider
                  label="Volumen del evento"
                  value={setting.volume}
                  onValueChange={(volume) => setSound(slot.id, { volume })}
                  step={0.05}
                  disabled={readonly}
                />
              </View>
              ...
              {!readonly && setting.uri ? (
                ...
                >
                  ...
                  <Text className="text-xs font-bold text-red-300/70">
                    Quitar sonido
                  </Text>
                </Pressable>
              ) : null}
