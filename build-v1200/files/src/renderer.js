...
const LuluWidgetDesign = window.LuluWidgetDesign;
const persistSettings = window.LuluSettingsPersistence.create((settings) => api.saveSettings(settings), () => state.settings, (settings) => { state.settings = settings; });
...
const RELEASE_NOTES = Object.freeze({
  '1.2.0': Object.freeze([
    Object.freeze({icon:'▧',title:'Tu estudio de widgets',text:'Un espacio propio para editar música, monedas, alertas, metas, regalos y juegos.'}),
    Object.freeze({icon:'Aa',title:'Más personalización',text:'Fondos y logos propios, tipografías, escala, marcos y sombra. Copia diseños entre widgets.'}),
    Object.freeze({icon:'✓',title:'Cambios que se conservan',text:'El guardado protege los ajustes al editar varios controles rápidamente.'}),
    Object.freeze({icon:'⌕',title:'Interfaz renovada',text:'Navegación más compacta, controles claros y búsqueda con Ctrl K.'})
  ]),
  '1.1.7': Object.freeze([
    Object.freeze({ icon:'!', title:'Comandos reconocidos', text:'Tolera espacios especiales, signos alternativos y acentos; incluye !true y !stop para controlar la música.' }),
    Object.freeze({ icon:'🌙', title:'Cobros confiables', text:'Un comando con costo activa Economía y nunca se ejecuta gratis por una configuración apagada.' }),
    Object.freeze({ icon:'◈', title:'Widgets bajo demanda', text:'Saldo, juegos, alertas y regalos sólo aparecen durante la acción que los activa.' }),
    Object.freeze({ icon:'♫', title:'Música estable', text:'YouTube y Spotify recuperan pausas inesperadas y siguen respetando una pausa manual.' })
  ]),
  '1.1.6': Object.freeze([
    Object.freeze({ icon:'🔒', title:'HTTPS automático', text:'Cada fuente crea su enlace seguro al abrir el apartado, sin pasos extra.' }),
    Object.freeze({ icon:'↻', title:'Recuperación integrada', text:'Si la red falla, Lulu muestra la causa real y vuelve a intentarlo.' }),
    Object.freeze({ icon:'◈', title:'Widgets sincronizados', text:'Juegos, música, monedas, alertas, metas y regalos publican su diseño correcto.' }),
    Object.freeze({ icon:'×', title:'Ventana protegida', text:'Cerrar, minimizar y maximizar siguen funcionando de forma independiente.' })
  ]),
  '1.1.5': Object.freeze([
...
function ensureStreamWidgetStyleEditors() {
  qsa('[data-widget-theme-gallery]').forEach((themeGallery) => {
    ...
    anchor.insertAdjacentHTML('afterend', `<section class="widget-style-editor" data-widget-style-editor="${type}"><header><span><strong>Tu diseño</strong><small>Personaliza la apariencia de esta fuente.</small></span><b data-widget-style-state>Usa el tema</b></header><div class="widget-style-color-grid"><label><span>Color principal</span><input type="color" data-widget-style-field="primaryColor"></label><label><span>Color secundario</span><input type="color" data-widget-style-field="secondaryColor"></label><label><span>Texto</span><input type="color" data-widget-style-field="textColor"></label><label><span>Fondo</span><input type="color" data-widget-style-field="backgroundColor"></label></div><div class="widget-style-range-grid"><label><span>Opacidad <output data-widget-style-output="backgroundOpacity">94%</output></span><input type="range" min="0" max="100" step="1" data-widget-style-field="backgroundOpacity"></label><label><span>Bordes <output data-widget-style-output="borderRadius">22 px</output></span><input type="range" min="0" max="48" step="1" data-widget-style-field="borderRadius"></label>${goalControl}</div>${widgetStudioFields(type)}<footer><small data-widget-style-status>Elige un color para activar tu diseño.</small><button type="button" class="ghost tiny" data-widget-style-reset>Restablecer</button></footer></section>`);
  });
}
...
function renderStreamWidgetStyleEditors() {
  ...
  qsa('[data-widget-style-editor]').forEach((editor) => {
    ...
    editor.querySelectorAll('[data-design-output]').forEach((output) => { output.textContent = style[output.dataset.designOutput] + (output.dataset.designOutput === 'scale' ? '%' : ' px'); });
    editor.querySelectorAll('[data-design-image-state]').forEach((label) => { label.textContent = style[label.dataset.designImageState] ? 'Imagen guardada en Lulu' : 'Sin imagen'; });
...
function collectRankingControls() {
  ...
  state.rankingSaveTimer=setTimeout(async()=>{try{await persistSettings();await api.refreshRanking(slot);if(Number($('rankingSlotInput')?.value||1)===slot)await refreshRankingInfo(slot,true);}catch(error){toast('No se guardó el ranking',error.message||String(error),'error');}},220);
}
  ...
  return (Array.isArray(state.settings.customCommands) ? state.settings.customCommands : [])
    ...
    .map((command) => {
      ...
      return {
        ...
        action: command.action === 'spotify' ? 'song' : (['response', 'tts', 'song', 'skip', 'resume', 'pause', 'sound', 'image', 'balance', 'revoke'].includes(command.action) ? command.action : 'response'),
...
function parseIncomingCommand(comment) {
  if (window.LuluCommandMatching?.parseCommandText) return window.LuluCommandMatching.parseCommandText(comment);
  const text = String(comment || '').trim();
  const match = text.match(/^!([^\s:;,?.]+)(?:\s*[:;,?.]\s*|\s+|$)([\s\S]*)$/u);
  return match ? { key:`!${match[1].toLowerCase()}`, remainder:String(match[2] || '').trim(), text } : null;
}

function incomingCommandKey(value) {
  if (window.LuluCommandMatching?.commandKey) return window.LuluCommandMatching.commandKey(value);
  return parseIncomingCommand(value)?.key || '';
}

function findCommand(comment) {
  const parsed = parseIncomingCommand(comment);
  if (!parsed) return null;
  const commands = normalizedCommands().filter((command) => command.enabled).sort((a, b) => b.trigger.length - a.trigger.length);
  const command = commands.find((item) => incomingCommandKey(item.trigger) === parsed.key);
  return command ? { ...command, matchedRemainder:parsed.remainder } : null;
}

function commandRemainder(comment, command) {
  if (typeof command?.matchedRemainder === 'string') return command.matchedRemainder;
  return parseIncomingCommand(comment)?.remainder || '';
}
...
function filterComment(message) {
  ...
  const prefix = String(state.settings.songPrefix || '!cancion');
  const knownCommand = findCommand(text);
  const parsedCommand = parseIncomingCommand(text);
  if (!state.settings.readCommands && parsedCommand && !knownCommand && parsedCommand.key !== incomingCommandKey(prefix)) {
...
function parseSongCommand(comment) {
  ...
  const prefixKey = incomingCommandKey(prefix);
  const enabledSongCommand = normalizedCommands().some((command) => command.enabled && command.action === 'song' && incomingCommandKey(command.trigger) === prefixKey);
  if (!prefix || !enabledSongCommand) return null;
  const parsed = parseIncomingCommand(comment);
  if (!parsed || parsed.key !== prefixKey) return null;
  return { query: parsed.remainder };
}
...
async function setActiveMusicPaused(paused) {
  return activeMusicProvider() === 'spotify'
    ? api.controlSpotify(paused ? 'pause' : 'play')
    : api.controlYouTube(paused ? 'pause' : 'play');
}
...
async function chargeCommand(command, message) {
  ...
  if (amount <= 0) return { ok:true, amount:0, transactionId:'' };
  if (!state.settings?.economyEnabled) {
    updateCommentResult(message.id, 'blocked', 'economía desactivada');
    toast('No se pudo cobrar', 'Activa Economía para usar comandos con costo.', 'error');
...
async function handleCustomCommand(command, message) {
  ...
  if (command.action === 'balance') {
    ...
    try {
      ...
      api.updateStreamWidget('wallet', {
        id:`wallet-${message.id}`, visible:true, expiresAt,
        user:normalizeUser(message.uniqueId), displayName:message.nickname || message.uniqueId,
        profilePictureUrl:message.profilePictureUrl || '', balance:Number(balance?.balance || 0),
        currencyName:String(state.settings.currencyName || 'Lunitas'), currencySymbol:String(state.settings.currencySymbol || '🌙')
      }).catch(()=>{});
  ...
  if (command.action === 'resume' || command.action === 'pause') {
    if (!hasActiveMusic()) { updateCommentResult(message.id, 'skipped', 'sin música activa'); return true; }
    const charge = await chargeCommand(command, message); if (!charge.ok) return true;
    const paused = command.action === 'pause';
    try {
      const result = await setActiveMusicPaused(paused);
      if (!result?.ok) throw new Error('El reproductor no está listo.');
      updateCommentResult(message.id, 'command');
      toast(paused ? 'Música pausada' : 'Música continuada', `Comando de ${message.nickname}`, 'success');
    } catch (error) {
      await refundCharge(charge, message, `Reembolso: no se pudo ${paused ? 'pausar' : 'continuar'} la música`);
      updateCommentResult(message.id, 'skipped', paused ? 'no se pudo pausar' : 'no se pudo continuar');
      toast('Control musical no disponible', error?.message || String(error), 'error');
    }
    return true;
  }
...
function parseLiveGameCommand(comment) {
  ...
  const parsed = parseIncomingCommand(comment);
  if (!parsed) return null;
  if (['!pedir','!hit'].includes(parsed.key)) return { game:'blackjack', action:'hit', args:parsed.remainder };
  if (['!plantar','!stand'].includes(parsed.key)) return { game:'blackjack', action:'stand', args:parsed.remainder };
  const commands = normalizedLiveGameCommands().filter((item)=>item.enabled).sort((a,b)=>b.trigger.length-a.trigger.length);
  const command = commands.find((item)=>incomingCommandKey(item.trigger)===parsed.key);
  if (!command) return null;
  return { game:command.id, action:'play', args:parsed.remainder, command };
}
...
async function handleAutomationEvent(event) {
  ...
  if (event.type === 'gift') state.giftStats = { ...(await api.updateGiftStats(state.giftStats, event)), widgetExpiresAt:Date.now()+12000 };
...
function saveCommandFromModal(event) {
  ...
  const duplicate = normalizedCommands().find((command) => incomingCommandKey(command.trigger) === incomingCommandKey(trigger) && command.id !== currentId);
  if (duplicate) {
  ...
  if (command.cost > 0 && !state.settings.economyEnabled) {
    state.settings.economyEnabled = true;
    if ($('economyEnabledInput')) $('economyEnabledInput').checked = true;
    toast('Economía activada', `${command.trigger} cobrará ${currencyText(command.cost)} por uso.`, 'success');
  }
  const commands = normalizedCommands();
...
const FEATURE_INDEX=[['Editor de widgets','Fondos, letras, marcos y vista previa','widgets'],['Conectar TikTok','Cuenta y estado del LIVE','account'],['Lulu Local','Voces sin Internet','voice','local'],['Voces por usuario','TTS personalizado','voice','users'],['Lectura inteligente','Unicode, emojis y CJK','voice','filters'],['Diccionario','Pronunciación personalizada','voice','dictionary'],['Música','YouTube, Spotify y cola','songs'],['Pantalla y overlays','OBS y TikTok LIVE Studio','rankings'],['Alertas y metas','Automatizaciones del LIVE','automations'],['Comandos','Respuestas, sonidos e imágenes','commands'],['Juegos','Interacciones del chat','games'],['Economía','Monedas y saldos','economy'],['Rendimiento','Módulos y memoria','settings','performance'],['Actualizaciones','Versión de Lulu Finity','settings','updates'],['Sobre Lulu','Información y contacto','about']];
...
async function init() {
  ...
  if (!state.settings.commandCostsEnforcedV117) {
    const hasPricedCommand = (Array.isArray(state.settings.customCommands) ? state.settings.customCommands : [])
      .some((command)=>command?.enabled===true && Math.max(0,Math.round(Number(command?.cost||0)))>0);
    if (hasPricedCommand) state.settings.economyEnabled = true;
    state.settings.commandCostsEnforcedV117 = true;
    migratedDefaultCommands = true;
  }
  if (!state.settings.musicControlCommandsMigratedV117) {
    const commands = Array.isArray(state.settings.customCommands) ? state.settings.customCommands : [];
    const hasCommand = (id, trigger) => commands.some((command)=>String(command?.id||'')===id || incomingCommandKey(command?.trigger)===incomingCommandKey(trigger));
    if (!hasCommand('music-resume','!true')) commands.push({id:'music-resume',trigger:'!true',action:'resume',response:'',permission:'all',enabled:true,cost:0});
    if (!hasCommand('music-pause','!stop')) commands.push({id:'music-pause',trigger:'!stop',action:'pause',response:'',permission:'all',enabled:false,cost:0});
    state.settings.customCommands = commands;
    state.settings.musicControlCommandsMigratedV117 = true;
    migratedDefaultCommands = true;
  }
  state.settings.economyRewards = normalizedEconomyRewards();
  state.settings.rankingOverlays = normalizedRankingOverlays();
  ...
  renderAutomationStudio();
  renderDashboardMusic();
  if (migratedV100||migratedTikTokVoices||migratedDefaultCommands || migratedVoiceCatalog || !state.settings.rankingsMigratedV016 || !state.settings.streamWidgetsMigratedV019) { state.settings.rankingsMigratedV016=true; state.settings.streamWidgetsMigratedV019=true; await persistSettings(); }
  setStatus({ status: 'disconnected' });
  setupEvents();
